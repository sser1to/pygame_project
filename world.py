import heapq
import json
import math
import random
from pathlib import Path

import pygame

from settings import (
    CANDLE_LIGHT_RADIUS,
    CANDLE_LIGHT_ALPHA,
    DARK_NOISE_OCTAVES,
    DARK_NOISE_PERSISTENCE,
    DARK_NOISE_SCALE,
    DARK_SAFE_RADIUS,
    DARK_SMOOTH_PASSES,
    DARK_TILE_FRACTION,
    DARK_OVERLAY_ALPHA,
    ITEM_APPLE,
    ITEM_CANDLE,
    ITEM_COAL,
    ITEM_LEVER,
    ITEM_MIN_DISTANCE_TILES,
    ITEM_SPAWN_ATTEMPTS,
    ITEM_SPAWN_SAMPLES,
    ITEM_STONE,
    ITEM_NOTE,
    ITEM_SPAWN_RELAX_SAME,
    ITEM_SPAWN_RELAX_SPAWN,
    LEVEL_COUNT,
    MONSTER_PATROL_COUNT,
    MONSTER_PATROL_SAMPLE_SIZE,
    MONSTER_SPAWN_MIN_DISTANCE,
    MONSTER_SPAWN_SAFE_RADIUS,
    SHADOW_BLUR_PASSES,
    SHADOW_CORNER_RADIUS,
    SHADOW_PAD,
    TILE_FLOOR,
    TILE_FURNACE,
    TILE_SPAWN,
    TILE_SIZE,
    TILE_WALL,
    TILE_WALL_TOP,
    SCREEN_HEIGHT,
    SCREEN_WIDTH,
    SOLID_TILES,
    COLD_MAX,
    COLD_RATE,
    COLD_RECOVERY_RATE,
    HUNGER_MAX,
    HUNGER_RATE,
    MONSTER_KILL_RADIUS,
    SPOTLIGHT_RADIUS,
    SPOTLIGHT_ALPHA,
    SPOTLIGHT_FEATHER,
    SPOTLIGHT_PULSE_SPEED,
    SPOTLIGHT_PULSE_AMOUNT,
    PLAYER_LIGHT_ALPHA,
    STONE_THROW_RANGE,
    EXIT_GLOW_PULSE_MIN,
    EXIT_GLOW_PULSE_AMP,
    EXIT_GLOW_PULSE_SPEED,
)
from assets import AssetManager


class Grid:
    def __init__(self, map_path, data=None):
        if data is not None:
            self.data = data
        else:
            self.data = self._load(map_path)
        self.rows = len(self.data)
        self.cols = len(self.data[0])

    @staticmethod
    def _load(path):
        rows = []
        with open(path, "r", encoding="utf-8") as handle:
            for line in handle:
                line = line.strip()
                if not line:
                    continue
                row = [int(value) for value in line.split()]
                rows.append(row)
        if not rows:
            raise ValueError("Map file is empty")
        width = max(len(row) for row in rows)
        for row in rows:
            if len(row) < width:
                row.extend([0] * (width - len(row)))
        return rows

    @property
    def map_size_px(self):
        return (self.cols * TILE_SIZE, self.rows * TILE_SIZE)

    def find_spawn_tile(self):
        for ty, row in enumerate(self.data):
            for tx, tile in enumerate(row):
                if tile == TILE_SPAWN:
                    self.data[ty][tx] = TILE_FLOOR
                    return (tx, ty)
        return (1, 1)

    def collect_floor_tiles(self):
        tiles = []
        for ty, row in enumerate(self.data):
            for tx, tile in enumerate(row):
                if tile == TILE_FLOOR:
                    tiles.append((tx, ty))
        return tiles

    @staticmethod
    def adjacent_tiles(tile):
        tx, ty = tile
        return [(tx + 1, ty), (tx - 1, ty), (tx, ty + 1), (tx, ty - 1)]

    def tile_in_bounds(self, tile):
        tx, ty = tile
        return 0 <= ty < self.rows and 0 <= tx < self.cols

    def tile_is_placeable(self, tile):
        if not self.tile_in_bounds(tile):
            return False
        tx, ty = tile
        return self.data[ty][tx] == TILE_FLOOR

    def tile_is_furnace(self, tile):
        if not self.tile_in_bounds(tile):
            return False
        tx, ty = tile
        return self.data[ty][tx] == TILE_FURNACE

    def tile_blocks_vision(self, tile):
        if not self.tile_in_bounds(tile):
            return True
        return self.data[tile[1]][tile[0]] in (TILE_WALL, TILE_WALL_TOP, TILE_FURNACE)

    def tile_is_walkable(self, tile):
        if not self.tile_in_bounds(tile):
            return False
        return self.data[tile[1]][tile[0]] not in SOLID_TILES

    @staticmethod
    def is_solid(tile_value):
        return tile_value in SOLID_TILES

    def iter_solid_tiles(self, rect, skip_tiles=None):
        left = max(0, rect.left // TILE_SIZE)
        right = min(self.cols - 1, (rect.right - 1) // TILE_SIZE)
        top = max(0, rect.top // TILE_SIZE)
        bottom = min(self.rows - 1, (rect.bottom - 1) // TILE_SIZE)
        for ty in range(top, bottom + 1):
            for tx in range(left, right + 1):
                tile_val = self.data[ty][tx]
                if self.is_solid(tile_val):
                    if skip_tiles is not None and tile_val in skip_tiles:
                        continue
                    yield pygame.Rect(tx * TILE_SIZE, ty * TILE_SIZE, TILE_SIZE, TILE_SIZE)

    @staticmethod
    def tile_to_world_center(tile):
        return (
            tile[0] * TILE_SIZE + TILE_SIZE / 2,
            tile[1] * TILE_SIZE + TILE_SIZE / 2,
        )

    @staticmethod
    def direction_to_offset(direction):
        if direction.length_squared() == 0:
            return (0, 0)
        if abs(direction.x) >= abs(direction.y):
            return (1 if direction.x > 0 else -1, 0)
        return (0, 1 if direction.y > 0 else -1)

    def get_wall_tiles_near(self, center_tile, radius):
        tiles = []
        for ty in range(max(0, center_tile[1] - radius), min(self.rows - 1, center_tile[1] + radius) + 1):
            for tx in range(max(0, center_tile[0] - radius), min(self.cols - 1, center_tile[0] + radius) + 1):
                if self.data[ty][tx] in (TILE_WALL, TILE_WALL_TOP):
                    tiles.append((tx, ty))
        return tiles

    def has_line_of_sight(self, start_pos, end_pos):
        start_tile = (int(start_pos[0] // TILE_SIZE), int(start_pos[1] // TILE_SIZE))
        end_tile = (int(end_pos[0] // TILE_SIZE), int(end_pos[1] // TILE_SIZE))
        for tile in self._bresenham_line(start_tile, end_tile):
            if tile == start_tile or tile == end_tile:
                continue
            if self.tile_blocks_vision(tile):
                return False
        return True

    @staticmethod
    def _bresenham_line(start_tile, end_tile):
        x0, y0 = start_tile
        x1, y1 = end_tile
        dx = abs(x1 - x0)
        dy = -abs(y1 - y0)
        sx = 1 if x0 < x1 else -1
        sy = 1 if y0 < y1 else -1
        err = dx + dy
        while True:
            yield (x0, y0)
            if x0 == x1 and y0 == y1:
                break
            e2 = 2 * err
            if e2 >= dy:
                err += dy
                x0 += sx
            if e2 <= dx:
                err += dx
                y0 += sy

    def get_furnace_rect(self, tile):
        tx, ty = tile
        if not self.tile_in_bounds(tile) or self.data[ty][tx] != TILE_FURNACE:
            return None
        left_tx = tx
        if self.tile_in_bounds((tx - 1, ty)) and self.data[ty][tx - 1] == TILE_FURNACE:
            left_tx = tx - 1
        width = TILE_SIZE * (
            2 if self.tile_in_bounds((left_tx + 1, ty)) and self.data[ty][left_tx + 1] == TILE_FURNACE else 1
        )
        return pygame.Rect(left_tx * TILE_SIZE, ty * TILE_SIZE, width, TILE_SIZE)

    def get_all_furnace_rects(self):
        visited = set()
        rects = []
        for ty in range(self.rows):
            for tx in range(self.cols):
                if (tx, ty) in visited:
                    continue
                if self.data[ty][tx] == TILE_FURNACE:
                    rect = self.get_furnace_rect((tx, ty))
                    if rect is not None:
                        rects.append(rect)
                        left_tx = rect.x // TILE_SIZE
                        for vx in range(rect.width // TILE_SIZE):
                            visited.add((left_tx + vx, ty))
        return rects

    @staticmethod
    def pick_arm_targets(wall_tiles, monster_pos):
        if not wall_tiles:
            return None, None
        world_positions = [Grid.tile_to_world_center(tile) for tile in wall_tiles]

        def distance_sq(pos):
            return (pos[0] - monster_pos.x) ** 2 + (pos[1] - monster_pos.y) ** 2

        left_candidates = [pos for pos in world_positions if pos[0] <= monster_pos.x]
        right_candidates = [pos for pos in world_positions if pos[0] >= monster_pos.x]
        left = min(left_candidates, key=distance_sq) if left_candidates else None
        right = min(right_candidates, key=distance_sq) if right_candidates else None
        if left is None:
            left = min(world_positions, key=distance_sq)
        if right is None:
            right = min((pos for pos in world_positions if pos != left), key=distance_sq, default=left)
        if left == right:
            alternate = min((pos for pos in world_positions if pos != left), key=distance_sq, default=left)
            right = alternate
        return left, right


class FlowField:
    @staticmethod
    def _wall_penalty(grid, tile):
        penalty = 0.0
        for neighbor in FlowField._neighbors8(tile):
            if not grid.tile_is_walkable(neighbor):
                penalty += 0.12
        return penalty

    @staticmethod
    def _neighbors8(tile):
        tx, ty = tile
        return [
            (tx - 1, ty - 1), (tx, ty - 1), (tx + 1, ty - 1),
            (tx - 1, ty), (tx + 1, ty),
            (tx - 1, ty + 1), (tx, ty + 1), (tx + 1, ty + 1),
        ]

    @staticmethod
    def build(grid, goals):
        rows = grid.rows
        cols = grid.cols
        inf = float("inf")
        cost = [[inf for _ in range(cols)] for _ in range(rows)]
        heap = []
        for goal_tile, bias in goals:
            if not grid.tile_is_walkable(goal_tile):
                continue
            tx, ty = goal_tile
            if bias < cost[ty][tx]:
                cost[ty][tx] = bias
                heapq.heappush(heap, (bias, tx, ty))
        while heap:
            current_cost, tx, ty = heapq.heappop(heap)
            if current_cost != cost[ty][tx]:
                continue
            for nx, ny in Grid.adjacent_tiles((tx, ty)):
                if not grid.tile_is_walkable((nx, ny)):
                    continue
                step_cost = 1.0 + FlowField._wall_penalty(grid, (nx, ny))
                next_cost = current_cost + step_cost
                if next_cost < cost[ny][nx]:
                    cost[ny][nx] = next_cost
                    heapq.heappush(heap, (next_cost, nx, ny))
        return cost

    @staticmethod
    def pick_step(flow_field, current_tile, rng):
        tx, ty = current_tile
        if ty < 0 or ty >= len(flow_field) or tx < 0 or tx >= len(flow_field[0]):
            return None
        current_cost = flow_field[ty][tx]
        if current_cost == float("inf"):
            return None
        candidates = []
        for nx, ny in Grid.adjacent_tiles(current_tile):
            if ny < 0 or ny >= len(flow_field) or nx < 0 or nx >= len(flow_field[0]):
                continue
            if flow_field[ny][nx] < current_cost:
                candidates.append((nx, ny))
        if not candidates:
            return None
        return rng.choice(candidates)


class DataManager:
    @staticmethod
    def load_levels(path):
        if not path.exists():
            return []
        with open(path, "r", encoding="utf-8") as handle:
            return json.load(handle)

    @staticmethod
    def get_level_config(levels, level_number):
        for level in levels:
            if level.get("level") == level_number:
                return level
        return {}

    @staticmethod
    def load_notes(path):
        if not path.exists():
            return []
        with open(path, "r", encoding="utf-8") as handle:
            return json.load(handle)

    @staticmethod
    def get_note(notes, level_number):
        for note in notes:
            if note.get("level") == level_number:
                return note
        return None


class SaveManager:
    SAVE_PATH = Path(__file__).with_name("save.json")

    @classmethod
    def load_progress(cls):
        default = 1
        try:
            if cls.SAVE_PATH.exists():
                with open(cls.SAVE_PATH, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    return max(1, min(int(data.get("unlocked_level", default)), LEVEL_COUNT))
        except (json.JSONDecodeError, OSError, ValueError):
            pass
        return default

    @classmethod
    def save_progress(cls, unlocked_level):
        try:
            with open(cls.SAVE_PATH, "w", encoding="utf-8") as f:
                json.dump({"unlocked_level": unlocked_level}, f)
        except OSError:
            pass


class Level:
    def __init__(self, grid, level_number, levels_data):
        self.grid = grid
        self.level_number = level_number

        config = DataManager.get_level_config(levels_data, level_number)
        items_config = config.get("items", {})
        objectives_config = config.get("objectives", {})
        debuffs_config = config.get("debuffs", {})
        abilities_config = config.get("monster_abilities", {})

        self.objectives_config = objectives_config
        self.items_config = items_config

        self.monster_hearing = abilities_config.get("hearing", False)
        self.monster_smell = abilities_config.get("smell", False)
        self.monster_vision = abilities_config.get("vision", False)
        self.monster_cloning = max(0, int(abilities_config.get("cloning", 0)))
        self.freeze_enabled = debuffs_config.get("freezing", False)
        self.hunger_enabled = debuffs_config.get("hunger", False)
        self.darkness_amount = debuffs_config.get("darkness_amount", 0)
        self.coal_required = max(0, int(objectives_config.get("coal", 0)))
        self.levers_required = max(0, int(objectives_config.get("levers", 0)))

        self.spawn_tile = grid.find_spawn_tile()
        self.dark_tiles = self._build_dark_tiles(self.spawn_tile) if self.darkness_amount > 0 else set()

        self.player = None
        self.monsters = []
        self.items = []
        self.stone_projectiles = []

        self.cold_value = 0.0
        self.hunger_value = 0.0
        self.dead = False
        self.coal_loaded = 0
        self.level_complete = False
        self.exit_was_unlocked = False
        self.exit_notification_timer = 0.0
        self.monster_kill_radius_sq = MONSTER_KILL_RADIUS * MONSTER_KILL_RADIUS

    def setup_entities(self, player, items, monsters):
        self.player = player
        self.items = items
        self.monsters = monsters

    def _build_dark_tiles(self, spawn_tile):
        floor_tiles = self.grid.collect_floor_tiles()
        if not floor_tiles:
            return set()

        rng = random.Random()
        noise = self._fractal_noise(rng)

        safe_radius_sq = DARK_SAFE_RADIUS * DARK_SAFE_RADIUS
        scores = []
        for ty in range(self.grid.rows):
            for tx in range(self.grid.cols):
                if self.grid.data[ty][tx] != TILE_FLOOR:
                    continue
                dx = tx - spawn_tile[0]
                dy = ty - spawn_tile[1]
                dist_sq = dx * dx + dy * dy
                if dist_sq <= safe_radius_sq:
                    continue
                edge_bias = (abs(dx) + abs(dy)) * 0.015
                jitter = rng.uniform(-0.05, 0.05)
                score = noise[ty][tx] + edge_bias + jitter
                scores.append((score, (tx, ty)))

        if not scores:
            return set()

        fraction = DARK_TILE_FRACTION * max(0, self.darkness_amount)
        total = min(len(scores), max(0, int(len(scores) * fraction)))
        if total <= 0:
            return set()

        scores.sort(key=lambda item: item[0], reverse=True)
        dark_tiles = {tile for _, tile in scores[:total]}

        for _ in range(max(1, DARK_SMOOTH_PASSES)):
            next_dark = set()
            for _, tile in scores:
                dark_neighbors = 0
                for neighbor in self._neighbors8(tile):
                    if not self.grid.tile_in_bounds(neighbor):
                        dark_neighbors += 1
                        continue
                    if self.grid.data[neighbor[1]][neighbor[0]] != TILE_FLOOR:
                        dark_neighbors += 1
                        continue
                    if neighbor in dark_tiles:
                        dark_neighbors += 1
                if tile in dark_tiles:
                    if dark_neighbors >= 3:
                        next_dark.add(tile)
                else:
                    if dark_neighbors >= 5:
                        next_dark.add(tile)
            dark_tiles = next_dark

        dark_tiles = self._filter_small_clusters(dark_tiles, 3)
        return dark_tiles

    @staticmethod
    def _neighbors8(tile):
        return FlowField._neighbors8(tile)

    @staticmethod
    def _lerp(a, b, t):
        return a + (b - a) * t

    @staticmethod
    def _smoothstep(t):
        return t * t * (3.0 - 2.0 * t)

    def _value_noise_layer(self, cols, rows, scale, rng):
        grid_cols = max(2, cols // scale + 2)
        grid_rows = max(2, rows // scale + 2)
        values = [[rng.random() for _ in range(grid_cols)] for _ in range(grid_rows)]
        layer = [[0.0 for _ in range(cols)] for _ in range(rows)]
        for y in range(rows):
            gy = y / float(scale)
            y0 = int(gy)
            y1 = min(y0 + 1, grid_rows - 1)
            ty = self._smoothstep(gy - y0)
            for x in range(cols):
                gx = x / float(scale)
                x0 = int(gx)
                x1 = min(x0 + 1, grid_cols - 1)
                tx = self._smoothstep(gx - x0)
                v00 = values[y0][x0]
                v10 = values[y0][x1]
                v01 = values[y1][x0]
                v11 = values[y1][x1]
                v0 = self._lerp(v00, v10, tx)
                v1 = self._lerp(v01, v11, tx)
                layer[y][x] = self._lerp(v0, v1, ty)
        return layer

    def _fractal_noise(self, rng):
        cols, rows = self.grid.cols, self.grid.rows
        noise = [[0.0 for _ in range(cols)] for _ in range(rows)]
        amplitude = 1.0
        total_amp = 0.0
        scale = max(1, DARK_NOISE_SCALE)
        for _ in range(max(1, DARK_NOISE_OCTAVES)):
            layer = self._value_noise_layer(cols, rows, scale, rng)
            for y in range(rows):
                for x in range(cols):
                    noise[y][x] += layer[y][x] * amplitude
            total_amp += amplitude
            amplitude *= DARK_NOISE_PERSISTENCE
            scale = max(1, int(scale * 0.5))
        if total_amp <= 0:
            return noise
        for y in range(rows):
            for x in range(cols):
                noise[y][x] /= total_amp
        return noise

    @staticmethod
    def _filter_small_clusters(tiles, min_size):
        if min_size <= 1 or not tiles:
            return tiles
        visited = set()
        result = set()
        for tile in tiles:
            if tile in visited:
                continue
            cluster = set()
            stack = [tile]
            while stack:
                current = stack.pop()
                if current in visited or current not in tiles:
                    continue
                visited.add(current)
                cluster.add(current)
                for neighbor in Grid.adjacent_tiles(current):
                    if neighbor in tiles and neighbor not in visited:
                        stack.append(neighbor)
            if len(cluster) >= min_size:
                result.update(cluster)
        return result

    # ─── level setup helpers ──────────────────────────────────────────────────

    def build_patrol_points(self):
        floor_tiles = self.grid.collect_floor_tiles()
        if not floor_tiles:
            return []

        rng = random.Random()
        points = [rng.choice(floor_tiles)]
        while len(points) < min(MONSTER_PATROL_COUNT, len(floor_tiles)):
            candidates = rng.sample(floor_tiles, min(MONSTER_PATROL_SAMPLE_SIZE, len(floor_tiles)))
            best_tile = None
            best_dist = -1
            for cand in candidates:
                dist = min((cand[0] - p[0]) ** 2 + (cand[1] - p[1]) ** 2 for p in points)
                if dist > best_dist:
                    best_dist = dist
                    best_tile = cand
            points.append(best_tile)

        return [Grid.tile_to_world_center(tile) for tile in points]

    def pick_monster_spawns(self):
        count = 1 + self.monster_cloning
        if count <= 0:
            return []
        floor_tiles = self.grid.collect_floor_tiles()
        candidates = [
            tile
            for tile in floor_tiles
            if abs(tile[0] - self.spawn_tile[0]) + abs(tile[1] - self.spawn_tile[1]) > MONSTER_SPAWN_SAFE_RADIUS
        ]
        if not candidates:
            candidates = floor_tiles[:]

        rng = random.Random()
        rng.shuffle(candidates)
        spawns = []

        def far_enough(tile):
            return all(
                abs(tile[0] - other[0]) + abs(tile[1] - other[1]) >= MONSTER_SPAWN_MIN_DISTANCE
                for other in spawns
            )

        for tile in candidates:
            if len(spawns) >= count:
                break
            if far_enough(tile):
                spawns.append(tile)

        if len(spawns) < count:
            for tile in candidates:
                if len(spawns) >= count:
                    break
                if tile not in spawns:
                    spawns.append(tile)

        return spawns[:count]

    def spawn_items(self):
        from entities import Item

        floor_tiles = self.grid.collect_floor_tiles()
        available = [tile for tile in floor_tiles if tile != self.spawn_tile]
        rng = random.Random()
        min_same_sq = ITEM_MIN_DISTANCE_TILES * ITEM_MIN_DISTANCE_TILES
        min_spawn_sq = ITEM_MIN_DISTANCE_TILES * ITEM_MIN_DISTANCE_TILES

        items = []
        placed_by_kind = {
            ITEM_COAL: [],
            ITEM_CANDLE: [],
            ITEM_APPLE: [],
            ITEM_LEVER: [],
            ITEM_STONE: [],
            ITEM_NOTE: [],
        }

        candle_count = self.items_config.get("candles", 1)
        stone_count = self.items_config.get("stones", 0)
        food_count = self.items_config.get("food", 0)
        coal_count = self.objectives_config.get("coal", 0)
        lever_count = self.objectives_config.get("levers", 0)
        spawn_plan = [
            (ITEM_COAL, coal_count),
            (ITEM_CANDLE, candle_count),
            (ITEM_APPLE, food_count),
            (ITEM_LEVER, lever_count),
            (ITEM_STONE, stone_count),
            (ITEM_NOTE, 1),
        ]

        for kind, count in spawn_plan:
            if count <= 0:
                continue
            current_same_sq = min_same_sq
            current_spawn_sq = min_spawn_sq
            for _ in range(count):
                candidate = None
                for _ in range(max(1, ITEM_SPAWN_ATTEMPTS)):
                    candidate = self._best_candidate(
                        available,
                        placed_by_kind[kind],
                        current_same_sq,
                        current_spawn_sq,
                        rng,
                    )
                    if candidate is not None:
                        break
                    if current_same_sq > 0:
                        current_same_sq = max(0, int(current_same_sq * ITEM_SPAWN_RELAX_SAME))
                    elif current_spawn_sq > 0:
                        current_spawn_sq = max(0, int(current_spawn_sq * ITEM_SPAWN_RELAX_SPAWN))
                if candidate is None:
                    if not available:
                        break
                    candidate = rng.choice(available)
                items.append(Item(kind, candidate))
                placed_by_kind[kind].append(candidate)
                if candidate in available:
                    available.remove(candidate)
        return items

    def _best_candidate(self, available, existing, min_same_sq, min_spawn_sq, rng):
        best = None
        best_score = -1.0
        if not available:
            return None
        samples = min(len(available), max(3, ITEM_SPAWN_SAMPLES))
        for _ in range(samples):
            cand = rng.choice(available)
            dist_spawn = self._tile_distance_sq(cand, self.spawn_tile)
            if dist_spawn < min_spawn_sq:
                continue
            if existing:
                closest_same = min(self._tile_distance_sq(cand, other) for other in existing)
                if closest_same < min_same_sq:
                    continue
            else:
                closest_same = float("inf")
            score = min(closest_same, dist_spawn)
            if score > best_score:
                best = cand
                best_score = score
        return best

    @staticmethod
    def _tile_distance_sq(a, b):
        dx = a[0] - b[0]
        dy = a[1] - b[1]
        return dx * dx + dy * dy

    @staticmethod
    def get_player_tile(player):
        return (player.rect.centerx // TILE_SIZE, player.rect.centery // TILE_SIZE)

    @staticmethod
    def player_on_tile(player, tile):
        tile_rect = pygame.Rect(tile[0] * TILE_SIZE, tile[1] * TILE_SIZE, TILE_SIZE, TILE_SIZE)
        return player.rect.colliderect(tile_rect)

    def get_light_tiles(self, center_tile, radius):
        if radius <= 0:
            return set()
        cx, cy = center_tile
        light_tiles = set()
        for dx in range(-radius, radius + 1):
            for dy in range(-radius, radius + 1):
                if max(abs(dx), abs(dy)) > radius:
                    continue
                tile = (cx + dx, cy + dy)
                light_tiles.add(tile)
        return light_tiles

    def _get_candle_tiles(self, radius):
        tiles = set()
        if self.player.carrying == ITEM_CANDLE:
            tiles.update(self.get_light_tiles(self.get_player_tile(self.player), radius))
        for item in self.items:
            if item.kind == ITEM_CANDLE:
                tiles.update(self.get_light_tiles(item.tile, radius))
        return tiles

    def get_candle_light_tiles(self):
        return self._get_candle_tiles(CANDLE_LIGHT_RADIUS)

    def get_candle_visibility_tiles(self):
        return self._get_candle_tiles(CANDLE_LIGHT_RADIUS + 1)

    def get_candle_centers(self):
        centers = []
        if self.player.carrying == ITEM_CANDLE:
            centers.append((self.player.rect.centerx, self.player.rect.centery))
        for item in self.items:
            if item.kind != ITEM_CANDLE:
                continue
            centers.append(
                (
                    item.tile[0] * TILE_SIZE + TILE_SIZE // 2,
                    item.tile[1] * TILE_SIZE + TILE_SIZE // 2,
                )
            )
        return centers

    def item_is_visible(self, item_tile):
        visibility_tiles = self.get_candle_visibility_tiles()
        return item_tile not in self.dark_tiles or item_tile in visibility_tiles

    @staticmethod
    def get_item_at(items, tile):
        for item in items:
            if item.tile == tile:
                return item
        return None

    def throw_stone(self, start_tile, direction):
        last_valid = start_tile
        for step in range(1, STONE_THROW_RANGE + 1):
            tile = (start_tile[0] + direction[0] * step, start_tile[1] + direction[1] * step)
            if not self.grid.tile_in_bounds(tile):
                break
            if self.grid.tile_blocks_vision(tile):
                break
            if self.get_item_at(self.items, tile) is not None:
                break
            last_valid = tile

        if last_valid == start_tile and self.get_item_at(self.items, last_valid) is not None:
            return None
        return last_valid

    def handle_interaction(self, stone_sprite, sfx, sound_events):
        from entities import Item, StoneProjectile

        player_tile = self.get_player_tile(self.player)
        offset = Grid.direction_to_offset(self.player.last_dir)
        target_tile = (player_tile[0] + offset[0], player_tile[1] + offset[1]) if offset != (0, 0) else None

        if self.player.carrying:
            if offset == (0, 0):
                return False, False, None
            if self.player.carrying == ITEM_COAL and target_tile and self.grid.tile_is_furnace(target_tile):
                self.player.carrying = None
                self._emit_sound(sound_events, target_tile)
                if sfx is not None:
                    sfx["furnace"].play()
                return False, True, None
            if self.player.carrying == ITEM_STONE:
                landing_tile = self.throw_stone(player_tile, offset)
                if landing_tile is not None:
                    if landing_tile == player_tile:
                        self.items.append(Item(ITEM_STONE, landing_tile))
                        self._emit_sound(sound_events, landing_tile)
                    else:
                        self.stone_projectiles.append(
                            StoneProjectile(
                                self.player.rect.center,
                                Grid.tile_to_world_center(landing_tile),
                                stone_sprite,
                                landing_tile,
                            )
                        )
                self.player.carrying = None
                return False, False, None
            if target_tile and self.grid.tile_is_placeable(target_tile) and self.get_item_at(self.items, target_tile) is None:
                self.items.append(Item(self.player.carrying, target_tile))
                self.player.carrying = None
            return False, False, None

        candidate_tiles = [player_tile] + Grid.adjacent_tiles(player_tile)
        for tile in candidate_tiles:
            item = self.get_item_at(self.items, tile)
            if item is None:
                continue
            if item.kind == ITEM_LEVER:
                if item.active:
                    continue
                item.active = True
                self._emit_sound(sound_events, tile)
                if sfx is not None:
                    sfx["lever"].play()
            elif item.kind == ITEM_APPLE:
                self.items.remove(item)
                if sfx is not None:
                    sfx["pickup"].play()
                return True, False, None
            elif item.kind == ITEM_NOTE:
                self.items.remove(item)
                if sfx is not None:
                    sfx["pickup"].play()
                return False, False, self.level_number
            else:
                self.player.carrying = item.kind
                self.items.remove(item)
                if sfx is not None:
                    sfx["pickup"].play()
            return False, False, None
        return False, False, None

    @staticmethod
    def _emit_sound(sound_events, tile):
        sound_events.append(pygame.Vector2(Grid.tile_to_world_center(tile)))

    def update(self, dt, sound_events):
        map_size_px = self.grid.map_size_px

        if not self.dead:
            skip_solid = {TILE_FURNACE} if (self.level_number == 7 and self.exit_was_unlocked) else None
            self.player.update(dt, self.grid, map_size_px, skip_solid)

            for projectile in self.stone_projectiles[:]:
                projectile.update(dt)
                if projectile.done:
                    from entities import Item
                    self.items.append(Item(ITEM_STONE, projectile.landing_tile))
                    self._emit_sound(sound_events, projectile.landing_tile)
                    self.stone_projectiles.remove(projectile)

            for monster in self.monsters:
                monster.update(
                    dt,
                    self.player.rect.center,
                    self.grid,
                    sound_events,
                    self.monster_hearing,
                    self.monster_smell,
                    self.monster_vision,
                )

            player_center = pygame.Vector2(self.player.rect.center)
            for monster in self.monsters:
                if (monster.pos - player_center).length_squared() <= self.monster_kill_radius_sq:
                    self.dead = True
                    break

        player_tile = self.get_player_tile(self.player)
        light_tiles = self.get_candle_light_tiles()

        if self.freeze_enabled:
            in_dark = player_tile in self.dark_tiles and player_tile not in light_tiles
            if in_dark:
                self.cold_value = min(COLD_MAX, self.cold_value + COLD_RATE * dt)
            else:
                self.cold_value = max(0.0, self.cold_value - COLD_RECOVERY_RATE * dt)

        if self.hunger_enabled:
            self.hunger_value = min(HUNGER_MAX, self.hunger_value + HUNGER_RATE * dt)

        if self.freeze_enabled and self.cold_value >= COLD_MAX:
            self.dead = True
        if self.hunger_enabled and self.hunger_value >= HUNGER_MAX:
            self.dead = True

        levers_active = sum(1 for item in self.items if item.kind == ITEM_LEVER and item.active)
        if not self.dead:
            coal_done = self.coal_required <= 0 or self.coal_loaded >= self.coal_required
            levers_done = self.levers_required <= 0 or levers_active >= self.levers_required

            if self.level_number == 7:
                now_unlocked = coal_done and levers_done
                if now_unlocked and not self.exit_was_unlocked:
                    self.exit_notification_timer = 0.0
                self.exit_was_unlocked = now_unlocked

                if now_unlocked:
                    for furnace_rect in self.grid.get_all_furnace_rects():
                        if self.player.rect.colliderect(furnace_rect):
                            self.level_complete = True
                            break
            else:
                self.level_complete = coal_done and levers_done

    @staticmethod
    def compute_camera(player_rect, map_size_px):
        map_w, map_h = map_size_px
        cam_x = player_rect.centerx - SCREEN_WIDTH // 2
        cam_y = player_rect.centery - SCREEN_HEIGHT // 2
        cam_x = max(0, min(cam_x, map_w - SCREEN_WIDTH))
        cam_y = max(0, min(cam_y, map_h - SCREEN_HEIGHT))
        return pygame.Vector2(cam_x, cam_y)

    def draw_world(self, screen, camera, assets, item_assets, item_outlines, dt):
        world_surface = pygame.Surface(screen.get_size())
        world_surface.fill((10, 10, 14))

        self._draw_map(world_surface, assets, camera)
        self._draw_items(world_surface, item_assets, camera)
        self._draw_projectiles(world_surface, camera)
        self.player.draw(world_surface, camera)
        self._draw_carried_item(world_surface, item_assets, camera)
        candle_centers = self.get_candle_centers()
        self._draw_spotlight(world_surface, candle_centers, camera, dt)
        for monster in self.monsters:
            monster.draw(world_surface, camera)
        self._draw_darkness(world_surface, candle_centers, camera)
        self._draw_item_outlines(world_surface, item_assets, item_outlines, camera)
        self._draw_furnace_outline(world_surface, camera)
        if self.level_number == 7 and self.exit_was_unlocked:
            self._draw_exit_glow(world_surface, camera)

        screen.blit(world_surface, (0, 0))

    def _draw_map(self, screen, assets, offset):
        skipped_furnace = set()
        for ty in range(self.grid.rows):
            for tx in range(self.grid.cols):
                tile = self.grid.data[ty][tx]
                if tile == 0:
                    continue
                world_x = tx * TILE_SIZE
                world_y = ty * TILE_SIZE
                if tile == TILE_FURNACE:
                    if (tx, ty) in skipped_furnace:
                        continue
                    if tx + 1 < self.grid.cols and self.grid.data[ty][tx + 1] == TILE_FURNACE:
                        screen.blit(assets["floor"], (world_x - offset.x, world_y - offset.y))
                        screen.blit(assets["floor"], (world_x + TILE_SIZE - offset.x, world_y - offset.y))
                        screen.blit(assets["furnace"], (world_x - offset.x, world_y - offset.y))
                        skipped_furnace.add((tx + 1, ty))
                        continue

                screen.blit(assets["floor"], (world_x - offset.x, world_y - offset.y))
                if tile == TILE_WALL:
                    screen.blit(assets["wall"], (world_x - offset.x, world_y - offset.y))
                elif tile == TILE_WALL_TOP:
                    screen.blit(assets["wall_top"], (world_x - offset.x, world_y - offset.y))
                elif tile == TILE_FURNACE:
                    screen.blit(assets["furnace_single"], (world_x - offset.x, world_y - offset.y))

    def _draw_items(self, screen, assets, offset):
        visibility_tiles = self.get_candle_visibility_tiles()
        for item in self.items:
            if item.tile in self.dark_tiles and item.tile not in visibility_tiles:
                continue
            item.draw(screen, assets, offset)

    def _draw_projectiles(self, screen, offset):
        for projectile in self.stone_projectiles:
            projectile.draw(screen, offset)

    def _draw_carried_item(self, screen, assets, offset):
        if not self.player.carrying:
            return
        sprite = assets[self.player.carrying]
        world_x = self.player.rect.centerx - sprite.get_width() // 2
        world_y = self.player.rect.centery - sprite.get_height() // 2
        screen.blit(sprite, (world_x - offset.x, world_y - offset.y))

    def _draw_spotlight(self, screen, candle_centers, camera, dt):
        t = pygame.time.get_ticks() / 1000.0
        sp = 1.0 - abs(math.sin(t * SPOTLIGHT_PULSE_SPEED)) * SPOTLIGHT_PULSE_AMOUNT

        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, int(SPOTLIGHT_ALPHA * sp)))

        center = (
            int(self.player.rect.centerx - camera.x),
            int(self.player.rect.centery - camera.y),
        )
        base_r = int(SPOTLIGHT_RADIUS * sp)
        for step in range(4):
            radius = base_r + step * (SPOTLIGHT_FEATHER // 3)
            alpha = max(0, int(SPOTLIGHT_ALPHA * sp - (step + 1) * (SPOTLIGHT_ALPHA // 5)))
            pygame.draw.circle(overlay, (0, 0, 0, alpha), center, radius)
        pygame.draw.circle(overlay, (0, 0, 0, int(PLAYER_LIGHT_ALPHA * sp)), center, base_r)

        candle_r = int((CANDLE_LIGHT_RADIUS + 0.5) * TILE_SIZE)
        for candle_center in candle_centers:
            screen_pos = (int(candle_center[0] - camera.x), int(candle_center[1] - camera.y))
            pygame.draw.circle(overlay, (0, 0, 0, CANDLE_LIGHT_ALPHA), screen_pos, candle_r)

        screen.blit(overlay, (0, 0))

    def _draw_darkness(self, screen, candle_centers, offset):
        darkness = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        for tx, ty in self.dark_tiles:
            world_x = tx * TILE_SIZE - offset.x
            world_y = ty * TILE_SIZE - offset.y
            rect = pygame.Rect(
                world_x - SHADOW_PAD,
                world_y - SHADOW_PAD,
                TILE_SIZE + SHADOW_PAD * 2,
                TILE_SIZE + SHADOW_PAD * 2,
            )
            pygame.draw.rect(
                darkness,
                (0, 0, 0, DARK_OVERLAY_ALPHA),
                rect,
                border_radius=SHADOW_CORNER_RADIUS + SHADOW_PAD,
            )

        candle_radius = int((CANDLE_LIGHT_RADIUS + 0.5) * TILE_SIZE)
        for candle_center in candle_centers:
            screen_pos = (int(candle_center[0] - offset.x), int(candle_center[1] - offset.y))
            pygame.draw.circle(darkness, (0, 0, 0, 0), screen_pos, candle_radius)

        darkness = AssetManager.soften_alpha_mask(darkness, SHADOW_BLUR_PASSES)
        screen.blit(darkness, (0, 0))

    def _draw_item_outlines(self, screen, assets, outlines, offset):
        from entities import get_item_outline_key, get_item_sprite

        player_tile = self.get_player_tile(self.player)
        can_pick = self.player.carrying is None
        pickable_tiles = [player_tile] + Grid.adjacent_tiles(player_tile)
        visibility_tiles = self.get_candle_visibility_tiles()
        candidate = None
        candidate_dist = None
        player_center = self.player.rect.center

        for item in self.items:
            if item.tile in self.dark_tiles and item.tile not in visibility_tiles:
                continue
            if item.tile not in pickable_tiles:
                continue
            if item.kind == ITEM_LEVER and item.active:
                continue
            if item.kind != ITEM_LEVER and not can_pick:
                continue
            tile_center = (
                item.tile[0] * TILE_SIZE + TILE_SIZE // 2,
                item.tile[1] * TILE_SIZE + TILE_SIZE // 2,
            )
            dist = (tile_center[0] - player_center[0]) ** 2 + (tile_center[1] - player_center[1]) ** 2
            if candidate is None or dist < candidate_dist:
                candidate = item
                candidate_dist = dist

        if candidate is None:
            return
        outline_key = get_item_outline_key(candidate)
        outline = outlines.get(outline_key)
        if outline is None:
            return
        sprite = get_item_sprite(candidate, assets)
        world_x = candidate.tile[0] * TILE_SIZE + (TILE_SIZE - sprite.get_width()) // 2
        world_y = candidate.tile[1] * TILE_SIZE + (TILE_SIZE - sprite.get_height()) // 2
        screen.blit(outline, (world_x - offset.x, world_y - offset.y))

    def _draw_furnace_outline(self, screen, offset):
        if self.player.carrying != ITEM_COAL:
            return
        offset_dir = Grid.direction_to_offset(self.player.last_dir)
        if offset_dir == (0, 0):
            return
        player_tile = self.get_player_tile(self.player)
        target = (player_tile[0] + offset_dir[0], player_tile[1] + offset_dir[1])
        rect = self.grid.get_furnace_rect(target)
        if rect is None:
            return
        outline_rect = pygame.Rect(rect.x - offset.x, rect.y - offset.y, rect.width, rect.height)
        pygame.draw.rect(screen, (80, 140, 200), outline_rect, 2)

    def _draw_exit_glow(self, screen, offset):
        rects = self.grid.get_all_furnace_rects()
        t = pygame.time.get_ticks() / 1000.0
        pulse = EXIT_GLOW_PULSE_MIN + EXIT_GLOW_PULSE_AMP * abs(math.sin(t * EXIT_GLOW_PULSE_SPEED))
        for rect in rects:
            screen_rect = pygame.Rect(rect.x - offset.x, rect.y - offset.y, rect.width, rect.height)
            glow = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
            alpha = int(220 * pulse)
            glow.fill((255, 255, 255, alpha))
            screen.blit(glow, screen_rect)
            border_alpha = int(200 * pulse)
            pygame.draw.rect(screen, (255, 255, 255, border_alpha), screen_rect, 3)
